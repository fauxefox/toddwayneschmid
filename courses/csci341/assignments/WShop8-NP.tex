...
\begin{problem}
    [3SAT]
    The set \(\mathit{Form}\) of all \emph{(three-propositional) formulas} is generated by the grammar 
    \[
        F \to p \mid q \mid r \mid (F \wedge F) \mid (F \vee F) \mid (\neg F)
    \]
    where \(p,q,r\) are the \emph{basic propositional formulas}.
    An \emph{assignment} is a function \(\alpha \colon \{p,q,r\} \to \{0,1\}\), indicating whether a \(p\), \(q\), and \(r\) are taken to be "false" or "true".
    ...
    \begin{itemize}
        ...
        \item A formula \(\varphi \in \mathit{Form}\) is \emph{satisfiable} if there exists an assignment \(\alpha \colon \{p,q,r\} \to \{0,1\}\) that makes \(\varphi = 1\) (i.e., true).
        We obtain the decision problem 
        \[
            \mathit{3SAT} = \{\varphi \in \mathit{Form} \mid \text{\(\varphi\) is satisfiable}\}
        \]
        Use your polynomial time algorithm in the previous part to show that (a string representation of) \(\mathit{3SAT}\) is in \(\mathsf{NP}\).
    \end{itemize}
